...
namespace ERP_Automation_Testing
{
    class CostCenter_Page
    {
        ...
        {
            Pages.CostCenterPage();
            time.Sleep(2000);

        }
	      
        public static void Add_CostCenter()
	    {                    
            Driver.FindElement(Add_Button).Click();
            time.Sleep(2000);
            Driver.FindElement(CostCenterName_TextBox).SendKeys(Data.CostCenter);
            Driver.FindElement(Save_Button).Click();
            time.Sleep(3000);
        }

        public static void Edit_CostCenter(string costCenter, string newName) 
	    {      
            Search(costCenter);
            Driver.FindElement(FirstItemEdit_Button).Click();
            time.Sleep(1000);
            Driver.FindElement(CostCenterName_TextBox).Clear();
            Driver.FindElement(CostCenterName_TextBox).SendKeys(newName);
            Driver.FindElement(Save_Button).Click();
            time.Sleep(3000);
        }

        public static void Delete_CostCenter(string costCenter) 
	    {       
            Search(costCenter);
